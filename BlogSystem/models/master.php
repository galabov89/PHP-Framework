...
namespace Models;
 ...
 class Master_Model{
     ...
     public function find_users_posts($postid){
         ...
         $statement =  "
                        SELECT postss.PostId, postss.Title, postss.Content, postss.UserName as 'PostOwner', commentss.Content as 'Comment',commentss.UserName as'CommentOwner'
                         FROM (
                             SELECT u.UserName , p.Content, p.Title , p.PostId
                             FROM `posts` p
                             LEFT JOIN `users` u on p.user_userid = u.userid
                             WHERE p.postid = $postid
                         ) as `postss` ,
                         (
                             SELECT u1.UserName, c1.Content
                             FROM `users` u1
                             LEFT JOIN `comments` c1 on c1.user_userid = u1.userid
                             WHERE c1.post_postid = $postid
                         ) as `commentss`
                        GROUP BY postss.PostId, postss.Title, postss.Content, postss.UserName, commentss.Content, commentss.UserName
         " ;
        ...
        // echo '///////////////////////////////////////////////////////////////////////////////////////';
         ...
         foreach ($result as $i => $row) {
             $comments[] =  array('Comment' => $result[$i]['Comment'],'CommentOwner' => $result[$i]['CommentOwner']);
